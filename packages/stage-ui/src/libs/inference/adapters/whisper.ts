...
import { DEVICE_LOSS_WASM_THRESHOLD, MAX_RESTARTS, MODEL_IDS, MODEL_NAMES, RESTART_DELAY_MS, TIMEOUTS } from '../constants'
...
export interface WhisperAdapter {
   ...
   */
  load: (
    modelIdOrOnProgress?: string | ((p: ProgressPayload) => void),
    onProgressOrOptions?: ((p: ProgressPayload) => void) | { signal?: AbortSignal },
...
export function createWhisperAdapter(workerUrl: string | URL): WhisperAdapter {
  ...
  async function load(
    ...
    const modelId = typeof modelIdOrOnProgress === 'string' ? modelIdOrOnProgress : undefined
    const onProgress = typeof modelIdOrOnProgress === 'function' ? modelIdOrOnProgress : (typeof onProgressOrOptions === 'function' ? onProgressOrOptions : undefined)
    const actualOptions = typeof onProgressOrOptions === 'object' ? onProgressOrOptions : options

    const targetModelId = modelId ?? MODEL_IDS.WHISPER
    const targetModelName = targetModelId.split('/').pop() || MODEL_NAMES.WHISPER
    ...
    throwIfAborted(actualOptions?.signal)
    return operationMutex.runExclusive(async () => {
      throwIfAborted(actualOptions?.signal)
      state = 'loading'
      updateInferenceStatus(targetModelName, { state: 'downloading', device: requestedDevice as any })

      return getLoadQueue().enqueue(targetModelName, LOAD_PRIORITY.ASR, async () => {
        throwIfAborted(actualOptions?.signal)
        ...
        }, actualOptions?.signal)

        w.postMessage({ type: 'load-model', requestId, modelId: targetModelId, device: requestedDevice })
        ...
        try {
          ...
          state = 'error'
          updateInferenceStatus(targetModelName, { state: 'error' })
          throw error
        }
        ...
        state = 'ready'
        updateInferenceStatus(targetModelName, { state: 'ready', device: actualDevice })
        onSuccess()
