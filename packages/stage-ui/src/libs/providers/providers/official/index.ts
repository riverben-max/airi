...
const OFFICIAL_CHAT_PROVIDER_ID = 'official-provider'
...
// Public-catalog default HTTP speech model id, populated by the HTTP speech
// provider's listModels(). The public catalog has no explicit default field, so
// the first enabled priced TTS model seeds empty/stale model selections.
...
type PublicModelCatalogCapability = 'chat' | 'tts' | 'image' | 'transcription'

interface PublicModelCatalogModel {
  id: string
  name?: string
  fluxPerCall: number
}

async function listOfficialCatalogModels(params: {
  capability: PublicModelCatalogCapability
  providerId: string
  label: string
}): Promise<ModelInfo[]> {
  const url = new URL(`${SERVER_URL}/api/v1/models`)
  url.searchParams.set('capability', params.capability)

  const res = await globalThis.fetch(url.toString(), { headers: authHeaders() })
  if (!res.ok)
    throw new Error(`official ${params.label} model catalog ${res.status}: ${await res.text().catch(() => '')}`.slice(0, 256))

  const data = await res.json() as { models?: PublicModelCatalogModel[] }
  if (!Array.isArray(data.models))
    throw new Error(`official ${params.label} model catalog returned malformed body`)

  return data.models.map((model) => {
    if (typeof model.id !== 'string' || model.id.length === 0 || typeof model.fluxPerCall !== 'number')
      throw new Error(`official ${params.label} model catalog returned malformed model`)

    return {
      id: model.id,
      name: typeof model.name === 'string' && model.name.length > 0 ? model.name : model.id,
      provider: params.providerId,
      description: `Flux cost: ${model.fluxPerCall}`,
    }
  })
}
...
export const providerOfficialChat = defineProvider({
  ...
  extraMethods: {
    listModels: async (): Promise<ModelInfo[]> => {
      return listOfficialCatalogModels({
        capability: 'chat',
        providerId: OFFICIAL_CHAT_PROVIDER_ID,
        label: 'chat',
      })
    },
  },
...
export const providerOfficialSpeech = defineProvider({
  ...
  extraMethods: {
    listModels: async (): Promise<ModelInfo[]> => {
      ...
      const models = await listOfficialCatalogModels({
        capability: 'tts',
        providerId: OFFICIAL_SPEECH_PROVIDER_ID,
        label: 'speech',
      })

      if (!models.length)
        throw new Error('official speech model catalog returned no TTS models; configure at least one enabled priced TTS model on the server')

      defaultSpeechModelId = models[0].id
      return models
    },
  ...
  extraMethods: {
    listModels: async (): Promise<ModelInfo[]> => {
      const models = await listOfficialCatalogModels({
        ...
        providerId: OFFICIAL_TRANSCRIPTION_PROVIDER_ID,
        label: 'transcription',
      })
      if (models.length > 0)
        return models
      ...
      // NOTICE:
      // Keep `auto` only as the ASR protocol alias fallback when the public
      // transcription catalog has not been configured yet. This is not a
      // billing price source; the server resolves the alias to a routerModelId
      // and charges according to that resolved catalog entry.
      // Remove this once every deployment seeds at least one priced
      // transcription model in `/api/v1/models?capability=transcription`.
      return [
        {
          id: 'auto',
          name: 'Auto',
