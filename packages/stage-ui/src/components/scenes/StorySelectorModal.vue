            ...
            <div v-if="selectedStory" class="min-h-0 flex flex-1 flex-col gap-4">
              ...
              <div class="flex flex-col gap-2">
                <div class="flex items-center justify-between">
                  <label class="text-xs text-white/70 font-semibold">Context Depth (Messages)</label>
                  <span class="text-xs text-rose-400 font-bold">{{ datingSimStore.settings.contextDepth }}</span>
                </div>
                <input
              ...
              <div class="flex flex-col gap-4 mt-2 border-t border-white/10 pt-4">
                ...
                <div v-if="datingSimStore.settings.gameMode === 'goal_driven'" class="flex items-center justify-between">
                  <label class="text-xs text-white/70 font-semibold">Show Choice Weights</label>
                  <button
                    type="button"
                    :class="[
                      'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                      datingSimStore.settings.showChoiceWeights ? 'bg-rose-500' : 'bg-neutral-600',
                    ]"
                    @click="datingSimStore.settings.showChoiceWeights = !datingSimStore.settings.showChoiceWeights"
                  >
                    <span
                      aria-hidden="true"
                      :class="[
                        'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                        datingSimStore.settings.showChoiceWeights ? 'translate-x-4' : 'translate-x-0',
                      ]"
                    />
                  </button>
                </div>
