...
import type { DataSettingsStatusPayload } from './status'
...
import LocalDataNotice from './components/localDataNotice.vue'
...
const importType = ref<'characters' | 'memory' | 'backgrounds'>('characters')
...
async function triggerExport(type: 'characters' | 'memory' | 'backgrounds') {
  try {
    let blob: Blob
    let filename: string
...
function triggerImportPicker(type: 'characters' | 'memory' | 'backgrounds') {
  importError.value = ''
