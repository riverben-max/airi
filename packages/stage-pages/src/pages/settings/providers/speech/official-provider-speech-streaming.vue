...
async function handleGenerateSpeech(input: string, voiceId: string, _useSSML: boolean): Promise<ArrayBuffer> {
  ...
  if (!requestedModel)
    throw new Error(t('settings.pages.providers.provider.official.streaming.errors.no_model'))
  ...
  if (slashIndex < 0) {
    throw new Error(t('settings.pages.providers.provider.official.streaming.errors.invalid_model', {
...
<template>
  ...
  >
    <ProviderSettingsContainer>
      ...
      <div v-else flex flex-col gap-6>
        ...
        <div :class="['space-y-3']">
          <Callout :label="t('settings.pages.providers.provider.official.streaming.model.title')">
            <p>{{ t('settings.pages.providers.provider.official.streaming.model.description') }}</p>
          </Callout>
          <Select
            v-model="model"
            :options="modelOptions"
            :disabled="modelsLoading"
            :placeholder="t('settings.pages.providers.provider.official.streaming.model.placeholder')"
          />
        </div>
  ...
  <div v-else :class="['p-8 text-center text-neutral-500']">
    {{ t('settings.pages.providers.provider.official.unavailable') }}
  </div>
</template>
