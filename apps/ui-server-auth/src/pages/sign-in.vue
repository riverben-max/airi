...
async function handleEmailSignIn(event: Event) {
  ...
  try {
    ...
    // cookie. For OIDC handoffs, keep the original authorize URL as the single
    // source of truth: better-auth's returned redirect can collapse the nested
    // callback into ui-server-auth's own route, which strands the code at
    // `/ui/callback` instead of the stage app's `/auth/callback`.
    window.location.href = oidcContinueURL.value || result.redirectURL || effectiveCallbackURL.value
  }
  catch (error) {
    errorMessage.value = describeAuthError(error) || t('server.auth.signIn.error.fallback')
  }
