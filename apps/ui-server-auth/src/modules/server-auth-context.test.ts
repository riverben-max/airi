...
describe('ui-server-auth bootstrap context', () => {
  ...
  it('allows self-hosted auth UI pages to use their own HTTPS origin as the API server', () => {
    expect(resolveStandaloneServerAuthContext(
      'https://airi.aifamily.vip/ui/sign-in?api_server_url=https%3A%2F%2Fairi.aifamily.vip',
      'https://api.airi.build',
    )).toEqual({
      apiServerUrl: 'https://airi.aifamily.vip',
      currentUrl: 'https://airi.aifamily.vip/ui/sign-in?api_server_url=https%3A%2F%2Fairi.aifamily.vip',
    })
  })

  it('defaults self-hosted HTTPS auth UI pages to their own origin when no API query is present', () => {
    expect(resolveStandaloneServerAuthContext(
      'https://airi.aifamily.vip/ui/sign-in',
      'https://api.airi.build',
    )).toEqual({
      apiServerUrl: 'https://airi.aifamily.vip',
      currentUrl: 'https://airi.aifamily.vip/ui/sign-in',
    })
  })
