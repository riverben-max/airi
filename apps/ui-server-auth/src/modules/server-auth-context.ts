...
const STANDALONE_AUTH_UI_ORIGINS = [
...
export function resolveStandaloneServerAuthContext(currentUrl: string, fallbackApiServerUrl: string): ServerAuthBootstrapContext | null {
  ...
  const resolvedApiServerUrl = apiServerUrl ?? normalizeSelfHostedApiServerUrl(url.origin)

  if (!resolvedApiServerUrl)
    return null

  return {
    apiServerUrl: resolvedApiServerUrl ?? fallbackApiServerUrl,
    currentUrl,
  }
}

function normalizeTrustedApiServerUrl(value: string | null, currentOrigin: string): string | null {
  if (!value)
    return null
  ...
  try {
    ...
    if (origin === currentOrigin)
      return normalizeSelfHostedApiServerUrl(origin)
...
function normalizeSelfHostedApiServerUrl(origin: string): string | null {
  if (STANDALONE_AUTH_UI_ORIGINS.includes(origin))
    return null
