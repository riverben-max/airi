# Email auth via SMTP / Resend (apps/server + apps/ui-server-auth)
...
1. 接入 **SMTP 优先、Resend fallback** 的 `apps/server` 统一邮件发送 service。
...
1. **注册路径**：用户敲 `/sign-up` → 填邮箱 + 密码 → 提交 → 进 `verify-email` 提示页 → 收邮件点链接 → `verify-email?token=...` 落地页提示成功 → 跳 `/ui/sign-in`。
2. **忘记密码路径**：用户在 `/ui/sign-in` 点 "Forgot password" → 进 `/forgot-password` 输邮箱 → 提交 → 提示已发送 → 用户点邮件链接 → `/reset-password?token=...` 输新密码 → 跳 `/ui/sign-in`。
3. **常规邮箱登录**：`/ui/sign-in` 输邮箱 + 密码 → 走 OIDC `loginPage` 流程把用户登入，返回上游 `/oauth/authorize`。

服务端为 magic link / change email 接好回调（避免功能闭包不齐一半），但前端 UI 留待后续。Service 拒绝静默吞错——发送失败要走错误响应让 Better Auth 把错抛回前端。

## 运维配置：注册验证邮件

新用户注册验证邮件走 SMTP 时，部署侧至少配置：
...
`SMTP_PORT=465` 通常搭配 `SMTP_SECURE=true`；`SMTP_PORT=587` 通常用 `SMTP_SECURE=false`，由 SMTP 服务端升级 STARTTLS。
...
- `apps/server/src/libs/env.ts`：
  ...
  - 通用发件人：`EMAIL_FROM_EMAIL`、`EMAIL_FROM_NAME`。
  - Resend fallback：`RESEND_API_KEY`、`RESEND_FROM_EMAIL`、`RESEND_FROM_NAME`。
...
- 管理后台动态配置邮件 provider。邮件 provider 仍只通过服务端环境变量配置。
- 邮箱 / 邮件模板的 i18n（先英文一个版本，后续补）。

## 关键决策

- **SMTP 优先**：只要设置 `SMTP_HOST`，`EmailService` 使用 SMTP 发送注册验证、重置密码、magic link、改邮箱确认和删除账号确认邮件；没有 `SMTP_HOST` 时才使用 Resend。
- **SMTP 鉴权**：`SMTP_USER` 和 `SMTP_PASSWORD` 必须同时设置；如果两者都为空，则按无鉴权 SMTP 连接。
- **超时**：SMTP transport 设置 10 秒 connection/greeting/socket timeout，避免注册请求无限等待邮件服务器。
...
- **OIDC `loginPage: '/ui/sign-in'` 不变**：sign-in 加表单后仍然走 `oauth/authorize → /ui/sign-in?... → 登录成功 → callbackURL 回 oauth/authorize`，不破坏现有流程。
