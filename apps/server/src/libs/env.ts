...
function optionalBooleanFromString(defaultValue: boolean, envKey: string) {
  return optional(
    pipe(
      string(),
      nonEmpty(`${envKey} must not be empty`),
      ...
      check(
        input => ['true', 'false', '1', '0', 'yes', 'no'].includes(input),
        `${envKey} must be true or false`,
      ),
      transform(input => input === 'true' || input === '1' || input === 'yes'),
    ),
    String(defaultValue),
  )
}
...
const EnvSchema = object({
  ...
  // Transactional email. SMTP is preferred when SMTP_HOST is configured;
  // otherwise Resend is used for backward-compatible deployments. Service boots
  // without either provider but email-driven auth flows throw at send-time.
  EMAIL_FROM_EMAIL: optional(string(), ''),
  EMAIL_FROM_NAME: optional(string(), ''),
  SMTP_HOST: optional(string(), ''),
  SMTP_PORT: optionalIntegerFromString(587, 'SMTP_PORT', 1),
  SMTP_SECURE: optionalBooleanFromString(false, 'SMTP_SECURE'),
  SMTP_USER: optional(string(), ''),
  SMTP_PASSWORD: optional(string(), ''),

  // Resend transactional email. Kept as the fallback provider for existing
  // deployments that already configured RESEND_API_KEY.
  ...
  // 易支付 (epay) — optional, gates epay routes gracefully when unset.
  // Real merchant checkout requires EPAY_API_URL, EPAY_PID, EPAY_KEY, and a
  // public callback base. Leave these empty for local non-payment development.
  EPAY_API_URL: optional(string(), ''),
  EPAY_PID: optional(string(), ''),
  EPAY_KEY: optional(string(), ''),
  // Optional public base URL used only for epay notify_url / return_url.
  // Falls back to API_SERVER_URL only when that URL is publicly reachable; real
  // epay integration must not use localhost or private-network callback URLs.
  EPAY_CALLBACK_BASE_URL: optional(string(), ''),
