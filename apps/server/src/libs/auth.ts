...
export function createAuth(
  ...
  return betterAuth({
    ...
    plugins: [
      ...
      oauthProvider({
        // NOTICE:
        // Send OIDC login redirects directly to the standalone auth UI.
        // Historical `/auth/*` routes still redirect old links, but an already
        // installed stage-web service worker can intercept `/auth/sign-in`
        // navigations before they reach Nginx. `/ui/*` is excluded from the PWA
        // navigation fallback, so the browser reaches the auth UI even while an
        // old worker is still controlling the page.
        loginPage: '/ui/sign-in',
