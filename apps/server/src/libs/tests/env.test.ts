...
describe('parseEnv', () => {
  ...
  it('parses SMTP email settings for transactional auth mail', () => {
    const env = parseEnv({
      ...baseEnv(),
      EMAIL_FROM_EMAIL: 'noreply@example.test',
      EMAIL_FROM_NAME: 'AIRI Mail',
    ...
    expect(env.EMAIL_FROM_EMAIL).toBe('noreply@example.test')
    expect(env.EMAIL_FROM_NAME).toBe('AIRI Mail')
    expect(env.SMTP_HOST).toBe('smtp.example.test')
    expect(env.SMTP_PORT).toBe(465)
    expect(env.SMTP_SECURE).toBe(true)
    expect(env.SMTP_USER).toBe('smtp-user')
