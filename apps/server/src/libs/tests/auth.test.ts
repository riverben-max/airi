...
describe('createAuth', () => {
  it('sends OIDC login redirects directly to the auth UI path', () => {
    ...
    const oauthPlugin = auth.options.plugins?.find(plugin => plugin.id === 'oauth-provider') as {
      options?: {
        loginPage?: string
      }
    } | undefined

    expect(oauthPlugin?.options?.loginPage).toBe('/ui/sign-in')
  })
  ...
  it('sends verification email automatically during sign-up', async () => {
    const email = {
      send: vi.fn(async () => {}),
      sendVerification: vi.fn(async () => {}),
      sendPasswordReset: vi.fn(async () => {}),
      sendMagicLink: vi.fn(async () => {}),
      sendChangeEmailConfirmation: vi.fn(async () => {}),
      sendDeleteAccountVerification: vi.fn(async () => {}),
    ...
    if (!sendVerificationEmail)
      throw new Error('Expected email verification sender to be configured')

    const url = 'http://localhost:3000/verify-email?token=token-1&callbackURL=%2F'
