...
import { bufferToString, toBufferLike } from './protocol'
...
export function createSessionState(
  ...
  let ttsFluxPerRequest = 0
  let ttsPricingSource: 'model_catalog' | 'config_fallback' = 'config_fallback'
  ...
  async function dialUpstream() {
    ...
    try {
      const catalogTtsPrice = await opts.providerCatalogService.getFixedModelPrice({
        capability: 'tts',
        routerModelId: modelLabel,
      })
      ttsFluxPerRequest = catalogTtsPrice ?? await opts.configKV.getOrThrow('TTS_FLUX_PER_REQUEST')
      ttsPricingSource = catalogTtsPrice == null ? 'config_fallback' : 'model_catalog'
    }
    catch (err) {
      log.withError(err).error('streaming TTS price resolution failed')
      closeWithError(1011, 'config_unavailable')
      return
    }

    // Pre-flight balance check: refuse before dialing if the user cannot
    // afford the fixed per-session TTS charge.
    try {
      const flux = await opts.fluxService.getFlux(userId)
      preflightFluxBalance = flux.flux
      if (flux.flux < ttsFluxPerRequest) {
    ...
    switch (evt.event) {
      case 'session.finished': {
        if (totalInputChars > 0)
          void billSession('session.finished')
        else
          finalize()
        break
      }
  ...
  async function validateStartFrame(frame: StreamingTtsStartFrame): Promise<boolean> {
    ...
    const upstreamConfig = unspeech?.streaming
    if (!upstreamConfig?.baseURL || upstreamConfig.keys.length === 0) {
      closeWithError(1008, 'streaming_tts_not_configured')
      return false
    }

    try {
      await opts.providerCatalogService.assertTtsModelEnabled(frame.model)
      await opts.providerCatalogService.assertTtsVoiceEnabled(frame.model, frame.voice)
    }
    catch (err) {
      if (err instanceof ApiError) {
        if (err.errorCode.startsWith('PROVIDER_CATALOG_TTS_MODEL_')) {
          closeWithError(1008, 'streaming_tts_model_not_enabled')
          return false
        }
        if (err.errorCode.startsWith('PROVIDER_CATALOG_TTS_VOICE_')) {
          closeWithError(1008, 'streaming_tts_voice_not_enabled')
          return false
        }
      }

      log.withError(err).warn('streaming tts provider catalog validation failed')
      closeWithError(1011, 'streaming_tts_catalog_validation_failed')
      return false
    }
  ...
  async function billSession(reason: string) {
    if (billed)
      return
    billed = true
    ...
    try {
      const result = await otelContext.with(trace.setSpan(otelContext.active(), span), () =>
        opts.billingService.consumeFluxForLLM({
          userId,
          amount: ttsFluxPerRequest,
          requestId,
          description: 'tts_request',
          model: modelLabel,
          capability: 'tts',
          pricingSource: ttsPricingSource,
        }))
      fluxConsumed = result.charged
      span.setAttribute(AIRI_ATTR_BILLING_FLUX_CONSUMED, fluxConsumed)
    }
    catch (err) {
      log.withError(err).withFields({ userId, amount: ttsFluxPerRequest, reason }).error('billing debit failed for streaming tts')
    ...
    void opts.productEventService.track({
      ...
      metadata: {
        ...
        billing_units: ttsFluxPerRequest,
        pricing_source: ttsPricingSource,
