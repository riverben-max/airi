...
const BillableModelCapabilitySchema = picklist(['chat', 'tts', 'image', 'transcription'])

const BillableModelCreateBodySchema = object({
  capability: BillableModelCapabilitySchema,
  ...
  fluxPerCall: pipe(number(), integer(), minValue(0)),
  priceEnabled: optional(boolean()),
})

const BillableModelUpdateBodySchema = object({
  capability: optional(BillableModelCapabilitySchema),
  routerModelId: optional(pipe(string(), maxLength(160))),
  displayName: optional(pipe(string(), maxLength(120))),
  enabled: optional(boolean()),
  displayOrder: optional(pipe(number(), integer(), minValue(0))),
  fluxPerCall: optional(pipe(number(), integer(), minValue(0))),
  priceEnabled: optional(boolean()),
})
...
export function createAdminProviderCatalogRoutes(deps: AdminProviderCatalogRoutesDeps) {
  return new Hono<HonoEnv>()
    ...
    .get('/models', async (c) => {
      ...
      const parsedCapability = capability == null ? null : safeParse(BillableModelCapabilitySchema, capability)
      if (parsedCapability != null && !parsedCapability.success)
        throw createBadRequestError('Invalid model capability', 'INVALID_MODEL_CAPABILITY', parseIssues(parsedCapability.issues))
      return c.json(await deps.service.listBillableModels(parsedCapability?.output))
    })
    .post('/models', async (c) => {
      const body = await readBody(c, BillableModelCreateBodySchema)
      return c.json(await deps.service.upsertBillableModel(body))
    })
    .patch('/models/:id', async (c) => {
      const body = await readBody(c, BillableModelUpdateBodySchema)
      const updated = await deps.service.updateBillableModel(c.req.param('id'), body)
      if (!updated)
        throw createNotFoundError('Billable model not found')
      return c.json(updated)
    })
    .delete('/models/:id', async (c) => {
      const deleted = await deps.service.deleteBillableModel(c.req.param('id'))
      if (!deleted)
        throw createNotFoundError('Billable model not found')
