...
import { createPaymentRequiredError, createServiceUnavailableError, createUnauthorizedError } from '../../utils/error'
...
export async function resolveOfficialAliyunNlsCredentialsFromConfig(input: {
  ...
  return { ...credentials, routerModelId: modelName }
}
...
export function createAudioTranscriptionStreamHandler(input: {
  ...
  return async function handleAudioTranscriptionStream(c: Context) {
    ...
    const fluxPerCall = await input.providerCatalogService.getFixedModelPrice({
      capability: 'transcription',
      routerModelId: credentials.routerModelId,
    })
    if (fluxPerCall == null) {
      throw createServiceUnavailableError(
        'Official ASR transcription model is not priced in the model catalog',
        'MODEL_PRICE_NOT_CONFIGURED',
        { capability: 'transcription', routerModelId: credentials.routerModelId },
      )
    }

    const flux = await input.fluxService.getFlux(session.user.id)
    ...
    return createAliyunNlsStreamResponse({
      ...
      credentials,
      async onTranscriptionStarted() {
        await input.billingService.consumeFluxForLLM({
          userId: session.user.id,
          amount: fluxPerCall,
          requestId,
          description: 'transcription_request',
          model: credentials.routerModelId,
          capability: 'transcription',
