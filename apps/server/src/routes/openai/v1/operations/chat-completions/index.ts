...
export function chatCompletions(deps: V1RouteDeps): GatewayCallback<'chat.completions'> {
  ...
  return async (context) => {
    ...
    const billingPolicy = await billing.authorizeChat(input.userId, aliasPlan.modelIds)
  ...
  ;(async () => {
    try {
      ...
      if (streamInterrupted) {
        ...
        const fluxConsumed = input.billing.priceChatUsage(usage, input.billingPolicy, input.requestModel)
        ...
        try {
          actualCharged = await input.billing.settleChat({
            ...
            model: input.requestModel,
            pricingSource: input.billingPolicy.pricingSource,
            stage: 'streaming',
            logger: input.logger,
            ...usage,
          })
        ...
        void input.deps.productEventService.track({
          ...
          metadata: {
            ...
            pricing_source: input.billingPolicy.pricingSource,
            stream: true,
          },
        })
