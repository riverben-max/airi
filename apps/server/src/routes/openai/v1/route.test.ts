...
function createMockProviderCatalogService(impl?: Partial<ProviderCatalogService>): ProviderCatalogService {
  ...
  return {
    ...
    upsertBillableModel: vi.fn(async input => ({
      id: 'billable-model-1',
    ...
    updateBillableModel: vi.fn(async (id, input) => ({
      id,
      capability: input.capability ?? 'chat',
      routerModelId: input.routerModelId ?? 'openai/gpt-5-mini',
      displayName: input.displayName ?? 'openai/gpt-5-mini',
      enabled: input.enabled ?? true,
      displayOrder: input.displayOrder ?? 0,
      fluxPerCall: input.fluxPerCall ?? 1,
      createdAt: new Date(),
      updatedAt: new Date(),
    })),
...
describe('v1CompletionsRoutes', () => {
  ...
  describe('pOST /api/v1/openai/chat/completions', () => {
    ...
    // Phase 2 customer billing policy is fixed Flux-per-call pricing.
    // Upstream token usage is still useful response metadata, but it must not
    // change the charged Flux amount for this commercial track.
    it('non-streaming completion ignores token usage and charges the fixed chat request price', async () => {
      ...
      const app = createTestApp(
        fluxService,
        createMockConfigKV({ FLUX_PER_REQUEST: 5 }),
        billingService,
        requestLogService,
      )
  ...
  describe('pOST /api/v1/audio/speech', () => {
    ...
    it('charges fixed Flux for a short TTS request', async () => {
      globalThis.fetch = vi.fn(async () => new Response(new Uint8Array([1]), {
        status: 200,
        headers: { 'Content-Type': 'audio/mpeg' },
      }))
      ...
      expect(ttsMeter.assertCanAfford).not.toHaveBeenCalled()
      expect(ttsMeter.accumulate).not.toHaveBeenCalled()
      expect(billingService.consumeFluxForLLM).toHaveBeenCalledTimes(1)
      expect(billingService.consumeFluxForLLM).toHaveBeenCalledWith(expect.objectContaining({
        userId: 'user-1',
        amount: 3,
     ...
     */
    it('does not multiply fixed TTS billing for Voice Pack requests', async () => {
      globalThis.fetch = vi.fn(async () => new Response(new Uint8Array([1]), {
        status: 200,
        headers: { 'Content-Type': 'audio/mpeg' },
      }))
    ...
    it('rejects manual TTS pre-flight when balance is below the fixed TTS rate', async () => {
      const billingService = createMockBillingService(2)
      const llmRouter = createMockLlmRouter()
      const app = createTestApp(
        createMockFluxService(2),
        createMockConfigKV({ TTS_FLUX_PER_REQUEST: 3 }),
        billingService,
        undefined,
        undefined,
        llmRouter,
    ...
    it('charges the same fixed Flux for long TTS input', async () => {
      globalThis.fetch = vi.fn(async () => new Response(new Uint8Array([1]), {
        status: 200,
        headers: { 'Content-Type': 'audio/mpeg' },
      }))
    ...
    // ROOT CAUSE:
    //
    // Before patch, `handleTTS` ran TTS billing outside any
    ...
    //
    // After patch:
    // billing + `span.setAttribute()` are wrapped in try/finally,
    ...
    it('tTS billing failure closes the span and surfaces error to onError (regression)', async () => {
      ...
      billingService.consumeFluxForLLM = vi.fn(async () => {
      ...
      expect(requestLogService.logRequest).not.toHaveBeenCalled()
      // consumeFluxForLLM was actually attempted (proves we walked into the billing
