...
import type { AdminSubscriptionPlanService, SubscriptionService } from './services/domain/subscriptions'
...
import { createDisabledPrivateSyncResponse, createDisabledPrivateSyncRoutes } from './routes/disabledPrivateSync'
import { createEpayRoutes } from './routes/epay'
import { createFluxRoutes } from './routes/flux'
import { createImageGenerationRoutes } from './routes/images'
import { createModelCatalogRoutes } from './routes/models'
...
import { createEpayService } from './services/domain/epay'
...
interface AppDeps {
  ...
  subscriptionService?: SubscriptionService & AdminSubscriptionPlanService
}
...
export async function buildApp(deps: AppDeps) {
  ...
  // NOTICE: Phase 1 keeps the legacy chat WebSocket implementation in the
  // tree for future separation work, but the customer-facing product stores
  // private chat, character, and memory data locally. Return a normal HTTP
  // response here instead of registering a WebSocket upgrade handler.
  app.get('/ws/chat', createDisabledPrivateSyncResponse)
  ...
  const subscriptionService = deps.subscriptionService ?? createSubscriptionService(deps.db)
  ...
  const builtApp = app
    ...
    /**
     * Private character sync is disabled for this product build.
     */
    .route('/api/v1/characters', createDisabledPrivateSyncRoutes())
    ...
    /**
     * Enabled billable model catalog for product clients.
     */
    .route('/api/v1/models', createModelCatalogRoutes(deps.providerCatalogService))
    ...
    /**
     * Private chat sync is disabled for this product build.
     */
    .route('/api/v1/chats', createDisabledPrivateSyncRoutes())
    ...
    /**
     * Official image generation gateway. Uses fixed model-catalog pricing
     * instead of prompt, size, or image-count metering.
     */
    .route('/api/v1/images', createImageGenerationRoutes({
      billingService: deps.billingService,
      configKV: deps.configKV,
      envelopeCrypto: deps.envelopeCrypto,
      fluxService: deps.fluxService,
    ...
    /**
     * 易支付 routes — optional, degrades gracefully when EPAY_* env vars unset.
     */
    .route('/api/v1/epay', createEpayRoutes(deps.epayService, deps.env))

    /**
     * Current subscription status for the authenticated subscription center.
     */
    .route('/api/v1/subscriptions', createSubscriptionRoutes(subscriptionService))
    ...
    /**
     * Admin subscription plan curation routes. Plans remain periodic
     * subscription products; disabling is soft-delete for audit safety.
     */
    .route('/api/admin/subscriptions', createAdminSubscriptionRoutes(subscriptionService))
...
export async function createApp() {
  ...
  const emailService = injeca.provide('services:email', {
    ...
    build: ({ dependsOn }) => createEmailService({
      ...
      fromEmail: dependsOn.env.EMAIL_FROM_EMAIL || dependsOn.env.RESEND_FROM_EMAIL,
      fromName: dependsOn.env.EMAIL_FROM_NAME || dependsOn.env.RESEND_FROM_NAME,
      smtp: dependsOn.env.SMTP_HOST
        ? {
            host: dependsOn.env.SMTP_HOST,
            port: dependsOn.env.SMTP_PORT,
            secure: dependsOn.env.SMTP_SECURE,
            user: dependsOn.env.SMTP_USER || undefined,
            password: dependsOn.env.SMTP_PASSWORD || undefined,
          }
        : undefined,
  ...
  const epayService = injeca.provide('services:epay', {
    dependsOn: { db, billingService, subscriptionService, env: parsedEnv },
    build: ({ dependsOn }) => createEpayService(dependsOn.db, dependsOn.billingService, dependsOn.subscriptionService, {
      apiUrl: dependsOn.env.EPAY_API_URL,
      pid: dependsOn.env.EPAY_PID,
      key: dependsOn.env.EPAY_KEY,
    }),
  })
  ...
  const { app, injectWebSocket } = await buildApp({
    ...
    epayService: resolved.epayService,
