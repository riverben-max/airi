...
export type BillableModelCapability = 'chat' | 'tts' | 'image' | 'transcription'
...
export const billableModels = pgTable(
  'billable_models',
  {
    id: text('id').primaryKey().$defaultFn(() => nanoid()),
    capability: text('capability').notNull().$type<BillableModelCapability>(),
    routerModelId: text('router_model_id').notNull(),
    displayName: text('display_name').notNull(),
    enabled: boolean('enabled').notNull().default(true),
    displayOrder: integer('display_order').notNull().default(0),
  ...
  table => [
    uniqueIndex('billable_models_capability_router_model_uidx').on(table.capability, table.routerModelId),
  ],
)

export const billableModelPrices = pgTable(
  'billable_model_prices',
  {
    id: text('id').primaryKey().$defaultFn(() => nanoid()),
    modelId: text('model_id').notNull().references(() => billableModels.id, { onDelete: 'cascade' }),
    fluxPerCall: integer('flux_per_call').notNull(),
    enabled: boolean('enabled').notNull().default(true),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
  },
  table => [
    uniqueIndex('billable_model_prices_model_uidx').on(table.modelId),
  ],
)
...
export type BillableModel = InferSelectModel<typeof billableModels>
export type NewBillableModel = InferInsertModel<typeof billableModels>
export type BillableModelPrice = InferSelectModel<typeof billableModelPrices>
export type NewBillableModelPrice = InferInsertModel<typeof billableModelPrices>
