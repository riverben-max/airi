...
export const imageUpstreamSchema = object({
  baseURL: pipe(string(), nonEmpty('image.upstream.baseURL must not be empty')),
  overrideModel: optional(string()),
  keys: pipe(array(keyEntrySchema), check(v => v.length >= 1, 'image.upstream.keys must contain at least 1 entry')),
  headerTemplate: optional(string(), 'Bearer {KEY}'),
  timeoutMs: optional(number()),
})

export const imageModelSchema = object({
  upstream: imageUpstreamSchema,
})
...
const ConfigEntrySchemas = {
  ...
  TTS_FLUX_PER_REQUEST: optional(number(), 3),
  FLUX_PER_1K_CHARS_TTS: optional(number(), 1),
  ...
  // NOTICE: Official image generation gateway MVP for the subscription track.
  // Size/quality cost differences are represented as different catalog models
  // with fixed Flux-per-call prices, not as runtime prompt/size/n pricing.
  DEFAULT_IMAGE_MODEL: pipe(string(), nonEmpty('DEFAULT_IMAGE_MODEL must not be empty')),
  IMAGE_ROUTER_CONFIG: optional(object({
    image: object({
      models: record(string(), imageModelSchema),
    }),
  })),
