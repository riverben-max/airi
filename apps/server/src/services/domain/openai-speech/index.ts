...
export function createOpenAiSpeechService(deps: OpenAiSpeechServiceDeps) {
  ...
  async function handleSpeechRequest(input: OpenAiSpeechRequest): Promise<Response> {
    ...
    const catalogTtsPrice = await deps.providerCatalogService.getFixedModelPrice({
      capability: 'tts',
      routerModelId: requestModel,
    })
    const ttsFluxPerRequest = catalogTtsPrice ?? await deps.configKV.getOrThrow('TTS_FLUX_PER_REQUEST')
    const ttsPricingSource = catalogTtsPrice == null ? 'config_fallback' : 'model_catalog'
    const shouldBill = inputText.length > 0
    ...
    if (shouldBill && flux.flux < ttsFluxPerRequest) {
      void deps.productEventService.track({
        userId: input.userId,
        feature: 'tts',
    ...
    void deps.productEventService.track({
      ...
      metadata: {
        http_status: response.status,
        input_chars: inputText.length,
        billing_units: shouldBill ? ttsFluxPerRequest : 0,
