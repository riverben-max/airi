...
export interface BillableModelUpdateInput {
  capability?: BillableModelCapability
...
export interface BillableModelWithPrice extends BillableModel {
...
export function createProviderCatalogService(db: Database) {
  ...
  return {
    async listBillableModels(capability?: BillableModelCapability): Promise<BillableModelWithPrice[]> {
      const rows = await db.select({
        model: billableModels,
        price: billableModelPrices.fluxPerCall,
        priceEnabled: billableModelPrices.enabled,
      })
        .from(billableModels)
        .leftJoin(billableModelPrices, eq(billableModelPrices.modelId, billableModels.id))
        .where(capability == null ? undefined : eq(billableModels.capability, capability))
        .orderBy(asc(billableModels.displayOrder), asc(billableModels.routerModelId))

      return rows.map(row => ({ ...row.model, fluxPerCall: row.price ?? null, priceEnabled: row.priceEnabled ?? false }))
    },

    async upsertBillableModel(input: {
      capability: BillableModelCapability
      routerModelId: string
      displayName?: string
      enabled?: boolean
      ...
      fluxPerCall: number
      priceEnabled?: boolean
    }): Promise<BillableModelWithPrice> {
      const now = new Date()
      const [model] = await db.insert(billableModels).values({
        capability: input.capability,
        routerModelId: input.routerModelId,
        displayName: input.displayName ?? input.routerModelId,
        enabled: input.enabled ?? true,
        displayOrder: input.displayOrder ?? 0,
      }).onConflictDoUpdate({
        target: [billableModels.capability, billableModels.routerModelId],
        set: {
          displayName: input.displayName ?? input.routerModelId,
          enabled: input.enabled ?? true,
          displayOrder: input.displayOrder ?? 0,
      ...
      const [price] = await db.insert(billableModelPrices).values({
        modelId: model.id,
        fluxPerCall: input.fluxPerCall,
        enabled: input.priceEnabled ?? true,
      }).onConflictDoUpdate({
        target: [billableModelPrices.modelId],
        set: {
          fluxPerCall: input.fluxPerCall,
          enabled: input.priceEnabled ?? true,
          updatedAt: now,
      ...
      return { ...model, fluxPerCall: price.fluxPerCall, priceEnabled: price.enabled }
    },

    async updateBillableModel(id: string, input: BillableModelUpdateInput): Promise<BillableModelWithPrice | null> {
      const now = new Date()
      const modelPatch: Partial<typeof billableModels.$inferInsert> = {}
      if (input.capability !== undefined)
        modelPatch.capability = input.capability
      if (input.routerModelId !== undefined)
        modelPatch.routerModelId = input.routerModelId
      if (input.displayName !== undefined)
        modelPatch.displayName = input.displayName
      if (input.enabled !== undefined)
        modelPatch.enabled = input.enabled
      if (input.displayOrder !== undefined)
        modelPatch.displayOrder = input.displayOrder

      let model = await db.query.billableModels.findFirst({ where: eq(billableModels.id, id) })
      if (!model)
        return null

      if (Object.keys(modelPatch).length > 0) {
        const [updated] = await db.update(billableModels)
          .set({ ...modelPatch, updatedAt: now })
          .where(eq(billableModels.id, id))
          .returning()
        model = updated
      }

      let price = await db.query.billableModelPrices.findFirst({ where: eq(billableModelPrices.modelId, id) })
      if (input.fluxPerCall !== undefined || input.priceEnabled !== undefined) {
        if (!price && input.fluxPerCall === undefined && input.priceEnabled === true) {
          throw catalogError(
            'fluxPerCall is required before enabling billable model pricing',
            'BILLABLE_MODEL_PRICE_REQUIRED',
            { id },
          )
        }

        const priceEnabled = input.priceEnabled ?? price?.enabled ?? true
        const fluxPerCall = input.fluxPerCall ?? price?.fluxPerCall ?? 0
        const pricePatch: Partial<typeof billableModelPrices.$inferInsert> = { updatedAt: now }
        if (input.fluxPerCall !== undefined)
          pricePatch.fluxPerCall = input.fluxPerCall
        if (input.priceEnabled !== undefined)
          pricePatch.enabled = input.priceEnabled

        const [updatedPrice] = await db.insert(billableModelPrices).values({
          modelId: id,
          fluxPerCall,
      ...
      return { ...model, fluxPerCall: price?.fluxPerCall ?? null, priceEnabled: price?.enabled ?? false }
    },

    async deleteBillableModel(id: string): Promise<BillableModelWithPrice | null> {
      const now = new Date()
      const model = await db.query.billableModels.findFirst({ where: eq(billableModels.id, id) })
      if (!model)
        return null

      const [updatedModel] = await db.update(billableModels)
      ...
      let price = await db.query.billableModelPrices.findFirst({ where: eq(billableModelPrices.modelId, id) })
      if (price) {
        const [updatedPrice] = await db.update(billableModelPrices)
          .set({ enabled: false, updatedAt: now })
          .where(eq(billableModelPrices.modelId, id))
          .returning()
        price = updatedPrice
      }
    ...
    async getFixedModelPrice(input: { capability: BillableModelCapability, routerModelId: string }): Promise<number | null> {
      const rows = await db.select({ fluxPerCall: billableModelPrices.fluxPerCall })
        .from(billableModels)
        .innerJoin(billableModelPrices, eq(billableModelPrices.modelId, billableModels.id))
        .where(and(
          eq(billableModels.capability, input.capability),
          eq(billableModels.routerModelId, input.routerModelId),
          eq(billableModels.enabled, true),
          eq(billableModelPrices.enabled, true),
        ))
        .limit(1)

      return rows[0]?.fluxPerCall ?? null
    },
